...
import { requestAddMultipleInventoryReceiving } from '../../../utils/Requests';
import IReceivedItem from '../../../types/IReceivedItem';
import IReceiving from '../../../types/IReceiving';

interface AddProductFromReceivingModalProps extends RouteComponentProps, HTMLAttributes<HTMLDivElement> {
...
const AddProductFromReceivingModalComponent: FunctionComponent<AddProductFromReceivingModalProps> = (props) => {
    ...
    const [quantity] = useState(props.receivedItems.quantity || 0);
    ...
    const [inventoryObj, setInventoryObj] = useState<IInventory>({
        productId: props.receivedItems.productId || 0,
        purchaseOrderId: props.receivedOrder.id || 0,
        serialNumber: '',
        condition: props.receivedItems.cud || 'Choose Condition',
    ...
    const inventoryValidation = (): boolean => {
        ...
        if (props.generateSkuSwitch == false) {
    ...
    const finish = async (isFinished: boolean = true) => {
        ...
        if (inventoryValidation()) {
            if (isFinished) {
                setInventoryObj({ ...inventoryObj, productId: props.receivedItems.productId || 0 });
...
export const AddProductFromReceivingModal = withRouter(AddProductFromReceivingModalComponent);